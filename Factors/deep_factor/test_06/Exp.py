...
class Exp:
    def __init__(self, params, params_net):
        ...
        if self.model_name == 'mlp':
            self.net = mlp(params_net)
        if self.model_name == 'lstm':
            self.net = lstm(params_net)
        ...
        while cur_cnt*self.single_num < len(date_lst_all):
            ...
            for i in tqdm(range(len(date_lst))):
                ...
                for file in os.listdir(date_dir):
                    ...
                    #结合k线因子
                    try:
                        data_kline = pd.read_csv(os.path.join('/home/largefile/public/zjwang/HF_factors/kline_feature/output', date, file)).drop(columns=['pre_close']).fillna(0).values
                        data_kline[np.isinf(data_kline)] = 0
                        data_kline_ = pd.read_csv(os.path.join(self.dir_y, date, file))[['high', 'low', 'volume', 'turnover']][-240:].fillna(0).values
                        data_kline_[np.isinf(data_kline_)] = 0
                        data = np.concatenate([data, data_kline, data_kline_], axis=1)
                    except:
