...
from model import FactorVAE, FactorVAE_Loss
import pickle
...
class Exp:
    ...
    def net_init(self):
        ...
        self.net = FactorVAE(self.params_net)
        self.optimizer = torch.optim.Adam(self.net.parameters(), lr=self.lr)

        #define loss
        self.criterion = FactorVAE_Loss()
        self.criterion_ic = Loss()
    ...
    def train(self, loss_type='mse'):
        ...
        for epoch in range(self.epoches):
            ...
            for batch_idx, (x, y) in enumerate(self.train_data):
                ...
                z_post, z_prior, y_rec, _ = self.net(x, y)
                loss = self.criterion(z_post, z_prior, y, y_rec)
                loss.backward()
                nn.utils.clip_grad_norm_(self.net.parameters(), max_norm=20, norm_type=2)
                self.optimizer.step()

                if batch_idx % self.print_num == 0:
                    _, _, ic_mean, _loss =  self._eval()
                    print(ic_mean, loss, _loss)
                    print(f'batch :{batch_idx}, train_loss: {loss: .6f}, eval_loss: {_loss: .6f}, rank_ic: {ic_mean: .6f}')
                    self.net.train()
    ...
    def _eval(self, final=False):
        ...
        for batch_idx, (x, y) in enumerate(self.eval_data):
            x, y = x.to(self.device), y.to(self.device)
            z_post, z_prior, y_rec, y_pred = self.net(x, y)
            loss = self.criterion(z_post, z_prior, y, y_rec)
            loss_es += loss
            y_pred_each_day.append(y_pred.cpu().detach().numpy())
            y_each_day.append(y.cpu().numpy())
            cnt += 1
            if batch_idx+1 in self.data_len_each_day:
                ...
                rankic = self.criterion_ic(y_pred_each_day, y_each_day, type='rank_ic', mean=False).detach().numpy()  #mean=False  ==> shape=[240//interval-2] 
        ...
        return rankic_es, rankic_es.mean()/rankic_es.std(), rankic_es.mean(), loss_es/cnt

    
    def eval(self):
        print('#####-------------------eval-------------------#####')
        rankic_es, icir, ic_mean, loss =  self._eval(final=True)
        print(f'eval set: loss: {loss: .6f}, eval_icir: {icir: .6f}, ic_mean: {ic_mean: .6f}')
