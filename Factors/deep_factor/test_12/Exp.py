    ...
    def get_data_mode(self,  mode='train'):       
        ...
        for i in tqdm(range(len(date_lst_all))):
            ...
            for file in os.listdir(date_dir):
                ...
                #结合k线因子
                if self.k:
                    columns = ['open', 'close', 'high', 'low', 'volume', 'turnover', 'vwap_1', 'vwap_5', 'vwap_10', 'vwap_30', 'vwap_60', 'vwap_120', 'pre_close']
                    if not self.k_len:
                        self.k_len = len(columns)
                    try:
