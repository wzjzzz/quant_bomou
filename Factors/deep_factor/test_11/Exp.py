...
class Exp:
    ...
    def shuffle_date_lst(self):
        ...
        self.test_date = self.date_lst[idx+1-self.back_days:-10]
...
#=============================================
    def test(self):
        self.shuffle_date_lst()
        with open(os.path.join(self.output_dir, 'scaler.pkl'), 'rb') as f:
            self.x_scaler = pickle.load(f)
        self.get_data_mode_tmp()
        self.net_init()
        self.net.load_state_dict(torch.load(os.path.join(self.output_dir, 'model.pkl')))
        self.net.eval()
        self._test(final=True)



    def _test(self, final=False):
        self.net.eval()
        y_pred_each_day = []
        filter_list_each_day = self.filter_list.all(axis=1)
        cur_batch_idx = 0
        cnt = 0
        for batch_idx, (x, y) in enumerate(self.test_data):
            ...
            if batch_idx+1 in self.data_len_each_day:
                ...
                if final:
                    stocks = np.array(stocks)[filter_tmp]
                    pd.DataFrame(y_pred_each_day.numpy(), index=stocks).to_csv(os.path.join(self.output_dir, '{}.csv'.format(self.test_date[cnt+self.back_days])))
    ...
    def get_data_mode_tmp(self, mode='test'):       
        date_lst_all = self.test_date
        ...
        for i in tqdm(range(len(date_lst_all))):
            ...
            if i + 1 < len(date_lst_all):
                furture_date = date_lst_all[i+1]
                future_df_limit = pd.read_csv(os.path.join(self.dir_f, 'limit_minbar_daily', 'limit_{}.csv'.format(furture_date)), index_col=0)
            date_dir = os.path.join(self.dir_x, date) 
            for file in os.listdir(date_dir):
                if file.split('.')[1] != self.filter_ins:
                    continue
                data = pd.read_csv(os.path.join(self.dir_x, date, file)).fillna(0).values
